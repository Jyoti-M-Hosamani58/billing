...
def printInvoice(request, invoice_no):
    ...
    # Calculate totals
    total_qty = items.aggregate(total_qty=Sum('quantity'))['total_qty'] or 0
    subtotal = items.aggregate(subtotal=Sum('price'))['subtotal'] or 0.0

    cgst_total = 0.0
    sgst_total = 0.0

    # Calculate CGST and SGST based on each item's GST rate, price, and quantity
    for item in items:
